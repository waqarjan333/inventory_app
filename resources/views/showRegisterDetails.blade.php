...
<div id="appCapsule">
    <div class="section mt-1">
        <div class="section-title">
            {{ $acc_id->acc_name }} - {{ $type }}
            <a href="{{ route('show.Register.Pay', [$acc_id->acc_id, $type]) }}" class="btn btn-primary btn-sm ms-1" style="float:right; margin-top:-5px;">Pay</a>
            <a href="{{ url('showRegister/'.$type) }}" class="btn btn-danger btn-sm" style="float:right; margin-top:-5px;">Back</a>
        </div>
                        ...
                        <div class="payment text-center" style="width: 25% !important;">
                            @if($accountArray['increase']!="")
                                <div class="price text-success">
                                    <strong>{{ number_format($accountArray['increase'], 2) }}</strong>
                                </div>
                            @elseif($accountArray['decrease']!="")
                                <div class="price text-danger">
                                    <strong>{{ number_format($accountArray['decrease'], 2) }}</strong>
                                </div>
                            @endif
                        </div>
                        @endif
