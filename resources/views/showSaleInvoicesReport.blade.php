        ...
        <div class="transactions mt-1">
            @if(count($report['records'])>0)
                @foreach ($report['records'] as  $record)
                    ...
                    @endif
                    @if($record['is_type']=="invoices")
                        <a href="{{ route('new.Sale.Invoice', $record['invoice_id']) }}" class="item" style="width:100%; padding: 20px 10px !important">
                            <div class="detail w-100">
                                <div class="w-100">
                                    <strong class="w-100">{{ $record['count']; }} - {{ $record['customer_name']; }}</strong>
                                    <p class="text-warning w-50 float-start text-left">INV-{{ $record['invoice_number']; }}</p>
                                    <p class="text-warning w-25 float-start text-center">{{ $carbon::parse($record['invoice_date'])->format('d M Y'); }}</p>
                                    <p class="text-warning w-25 float-end text-end">{{ $record['invoice_total']; }}</p>
                                </div>
                            </div>
                        </a>
                    @endif
                        ...
                        <a href="#" class="item bg-primary" style="width:100%; padding: 5px 10px !important">
                            <div class="detail w-100">
                                <div class="w-100">
                                    ...
                                    <p class="text-white w-30 float-end">{{ $record['group_sale']; }}</p>
                                </div>
                            </div>
                        </a>
                    @endif
                        ...
                        <a href="#" class="item bg-warning" style="width:100%; padding: 20px 10px !important">
                            <div class="detail w-100">
                                <div class="w-100">
                                    ...
                                    <p class="text-white w-30 float-end">{{ $record['grand_total_sale']; }}</p>
                                </div>
                            </div>
                        </a>
                    @endif
