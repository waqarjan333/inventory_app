        ...
        <div class="transactions mt-1">
            @if(count($report['records'])>0)
                @foreach ($report['records'] as  $record)
                    @if($record['is_type']=="represntative")
                        <a href="#" class="item bg-secondary" style="padding: 5px 10px !important">
                            <div class="detail">
                                <div><strong class="text-white">{{ $record['represntative_name'] }}</strong></div>
                            </div>
                        </a>
                    @endif
                    ...
                    @endif
                    @if($record['is_type']=="rep_total")
                        <a href="#" class="item bg-primary" style="width:100%; padding: 5px 10px !important">
                            <div class="detail w-100">
                                <div class="w-100">
                                    <strong class="w-70 text-white float-start">{{ $record['rep_total']; }}</strong>
                                    <p class="text-white w-30 float-end">{{ $record['rep_amount']; }}</p>
                                </div>
                            </div>
                        </a>
                    @endif
